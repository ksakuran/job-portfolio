...
const Slide = () => {
  return (
    ...
    >
      <SwiperSlide><h4 className='slider-title'>Mojo</h4><p className='slider-p'>A mood journaling application built with React, CSS (SCSS), JSX, and Axios for the front-end, and Express for the back-end with a PSQL DB. We used multiple API's and libraries like Chart.js and Spotify. My favourite part of working on this project was creating the dreamy design.</p><a onClick={() => window.open('https://github.com/ksakuran/MoJo', '_blank')}><img className='slider-img' src='mojo.png' alt='mojo'/></a></SwiperSlide>
      <SwiperSlide><h4 className='slider-title'>Treasure Maps</h4><p className='slider-p'>A single page full stack web application. Built using jQuery and AJAX for front-end, and Node.js and Express for back-end. We used the Leaflet API for this project.</p><a onClick={()=> window.open('https://github.com/ksakuran/treasure-map', '_blank')}><img className='slider-img' src='treasure-maps.png' alt='treasure-maps'/></a></SwiperSlide>
